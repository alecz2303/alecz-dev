...
<article class="case-study">
    ...
    <section class="case-showcase" aria-labelledby="showcase-title" data-reveal>
        ...
        @if ($galleryMedia->isNotEmpty())
            <div class="case-media-gallery" aria-label="{{ __('ui.case.gallery', ['project'=>$project['name']]) }}">@foreach ($galleryMedia as $item)<figure class="case-media-item"><img src="{{ asset($item['src']) }}" alt="{{ $item['alt'] ?? $project['name'] }}" loading="lazy">@if (!empty($item['caption']))<figcaption>{{ $item['caption'] }}</figcaption>@endif</figure>@endforeach</div>
        @endif
    </section>
    ...
    <nav class="case-pagination" aria-label="{{ $case['pagination'] }}" data-reveal>
        @if ($previous)<a href="{{ route($projectRoute, $previous['slug']) }}"><span>{{ $case['previous'] }}</span><strong>{{ $previous['name'] }}</strong></a>@else<span></span>@endif
        @if ($next)<a class="case-next" href="{{ route($projectRoute, $next['slug']) }}"><span>{{ $case['next'] }}</span><strong>{{ $next['name'] }}</strong></a>@endif
    </nav>
</article>
@endsection
